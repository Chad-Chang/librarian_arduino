...
void CEncoder::CurrentVel()
{  
  if (millis() - prevMillis > ENCODER_INTERVAL) {
...
void CEncoder::CurrentVel_FakeSign(float time_interval)
{ 
  rpm = 60./((double)ENCODER_INTERVAL/1000.) * (double)pulse_count / (13. * 4.) / MOTOR_RATIO;  // calculate RPM
  ang_vel = rpm * RPMtoRAD;  // calculate rad/s
...
void CEncoder::MoveEncoder(uint16_t position)
{
  ...
  if (position-5<cur_pulse_count && position+5>cur_pulse_count)
  {
    pwm = 0;
  }
...
void CEncoder::MoveEncoder_mm(uint16_t distance)
{
  ...
  mm_position = int((float)cur_pulse_count/1000*27);
