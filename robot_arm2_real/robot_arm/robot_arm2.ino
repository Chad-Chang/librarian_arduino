...
// ////////////ball_screw////////////////////////////////////////////////////////////////
//   if(ball_screw_only){
//     CMotor_B.MoveEncoder_mm(ball_mm); // CMotor_B.input_B 이거 대신 너가 원하는 숫자나 변수 넣으면 그 mm로 움직일거임. (절대좌표) 범위 0~148
...
//   //   cservo.gripperClose();
//   // }
//   // 로봇팔을 작동할때만 Last_signal을 변하하게함.
//   if(robot_op){ // 로봇팔 구동중
//     Last_signal = "0";
    
//     if(robot.status == bscrew_mode){  // 시작신호 4번 
//       cservo.grab();  // 얼마나 벌릴지 cservo.target에 넣어주기 default 0
//       cservo.target = 60; 
//       cservo.gripperOpen();
...
//           digitalWrite(motorDirB, CMotor_B.dir);
//           analogWrite(motorPwmB, CMotor_B.pwm);
//           if(CMotor_B.arrival){Serial.println("/5/");Serial1.println("/5/"); robot.status = 5; forward = true;}
//         }
//       }
//     }
//     else if(robot.status == end_rob){ //끝났을때 2 
//       robot.robot_run(packet, jnt_ang);
//       cservo.gripperOpen();
//       Serial.println("status end");
//       CMotor_B.MoveEncoder_mm(0); // CMotor_B.input_B 이거 대신 너가 원하는 숫자나 변수 넣으면 그 mm로 움직일거임. (절대좌표) 범위 0~148
//           // 이 아래는 볼 스크류 모터 움직이는 부분
//       digitalWrite(motorDirB, CMotor_B.dir);
//       analogWrite(motorPwmB, CMotor_B.pwm); 
//       if(CMotor_B.arrival){robot_op = false; forward = false;Last_signal = String(end_rob);}
//     }
//     else if(robot.status == BS_back){ // ball screw를 돌려 뒤로 빼는 과정  24
//       robot.robot_run(packet,jnt_ang);
//       CMotor_B.MoveEncoder_mm(20); // CMotor_B.input_B 이거 대신 너가 원하는 숫자나 변수 넣으면 그 mm로 움직일거임. (절대좌표) 범위 0~148
//       digitalWrite(motorDirB, CMotor_B.dir);
//       analogWrite(motorPwmB, CMotor_B.pwm);
//       if(CMotor_B.arrival){Serial1.println("/s/");}
//     }
//     else if(robot.status == grabb){ //잡는 상태일 때 12
...
//     else if(robot.status == shelve1_f||robot.status == shelve2_f){ // 17, 19 
//       robot.robot_run(packet, jnt_ang);
//       cservo.grab();  // 얼마나 벌릴지 cservo.target에 넣어주기 default 0
//       cservo.target = 50; 
//       cservo.gripperOpen();
...
//     }
//     else if(robot.status == read_fail){ // 책 못읽었을때: 1
